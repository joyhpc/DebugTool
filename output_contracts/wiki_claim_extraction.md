...
|---|---|---|---|
| S1 | ../my-wiki/wiki/... | project_note/datasheet/schematic/debug_record | contains target conditioner enable behavior |
...
|---|---|---|---|---|---|---|
| KC1 | target conditioner enable input is active-low | ../my-wiki/wiki/... | datasheet | high | specific part and revision | board-level enable voltage |

## Similar Problem Candidates Table

| case_id | source_ref | match_axis | transferable_lesson | limits | action_impact |
|---|---|---|---|---|---|
| SIM1 | training/dataset_1000/mipi_debug_queue.yaml:MIPI-002 | no-output video bridge debug | split source/bridge/panel with test pattern and status registers | similar interface family, not direct target evidence | add isolation branch |
